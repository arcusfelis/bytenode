 ...
 * @param   {string} filename Filename to show in stacktraces.
 * @returns {Buffer} The generated bytecode.
 */
const compileCode = function (javascriptCode, filename) {
...
const compileFile = function (args, output) {
  ...
  if (compileAsModule) {
    bytecodeBuffer = compileCode(Module.wrap(javascriptCode.replace(/^#!.*/, '')), filename);
  } else {
    bytecodeBuffer = compileCode(javascriptCode.replace(/^#!.*/, ''), filename);
  }
